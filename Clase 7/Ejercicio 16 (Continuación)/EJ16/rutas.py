...
# POST Usuarios Endpoint - Verifica si los Usuarios estan en la Colección
@router.post("/verify")
async def verify_user(user_name: str):
    user = collection_name.find_one({"nombre": user_name})
    if (user):
        serialized_user = individual_serializer(collection_name.find_one({"nombre": user_name}))
        return {"nombre": serialized_user["nombre"], "apellido": serialized_user["apellido"]}
    else:
        return "DENEGADO"
