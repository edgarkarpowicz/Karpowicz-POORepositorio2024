...
bool AdminDB::validarUsuario(QString tabla, QString usuario, QString clave)
{
    ...
    if ( db.open() )  {
        ...
        QSqlQuery query = db.exec( "SELECT usuario, clave FROM " + tabla + " WHERE usuario = '" +
...
std::vector<Rectangulo> AdminDB::recuperarDatosRectangulos(QString tabla) {
    std::vector<Rectangulo> vector;
    int x, y, ancho, alto;
    if ( db.open() )  {
        qDebug() << "Consiguiendo todos los Rectangulos...";
        QSqlQuery query = db.exec( "SELECT * FROM " + tabla);

        while (query.next()) {
            qDebug() << "Convirtiendo datos de los Rectangulos en Rectangulos";
            x = query.value(0).toInt();
            y = query.value(1).toInt();
            ancho = query.value(2).toInt();
            alto = query.value(3).toInt();

            Rectangulo rect(x, y, ancho, alto);

            vector.push_back(rect);
        }

        qDebug() << "Devolviendo Rectangulos...";
        emit rectangulosRecuperados();
        return vector;
    } else {
        qDebug() << "DB no esta abierta...";
        return vector;
    }
}
